...
def findDiskSize(filenames, method='fraction', ztol=1e-3):
    SIZES = np.array([])
    for filename in filenames:
        ...
        if(method == 'fraction'):
            ...
            sortOrder = np.argsort(R)
            R = R[sortOrder]
            Rad = R[int(fraction*len(R))]
            SIZES = np.append(SIZES, Rad * udist / (7 * 6.96e10))
            
        if(method == 'kep'):
            R = np.sqrt(partcoords[:,0]**2 + partcoords[:,1]**2) * udist * 0.01
            sortOrder = np.argsort(R)
            R = R[sortOrder]
            partvels = partvels[sortOrder] * udist * 0.01 / utime
            for i in range(len(R)):
                if(np.abs(partcoords[i,2]) < ztol):
                    Rad = R[i]
                    speed = np.sqrt(np.sum(partvels[i]**2))
                    escape = np.sqrt(2 * 6.67e-11 * 13 * 2e30 / R[i])
#                    print speed, kep
                    if(speed > escape):
#                        print R[i] / 49e8
                        break
            SIZES = np.append(SIZES, Rad / (7 * 6.96e8))
    return SIZES
    
def findPeriastron(filenames):
    seps = np.array([])
    for filename in filenames:
        diskname = filename[:-26] + "disk001"
        IPname = filename[:-11] + "IPDISK" + filename[-5:-2]
        Becoords, NScoords, partcoords, partvels, rho, u, num_parts, time, umass, udist, utime = readFILES(filename, diskname, IPname)
#        print (Becoords - NScoords)**2
        seps = np.append(seps, NScoords[0])#np.sqrt(np.sum((Becoords-NScoords)**2)))
#    print seps
    return np.min(seps), filenames[np.argmin(seps)]
    
def findBinaryPhase(baseName, orbit, colour):#filename, 
    NSx = np.array([])
    filenames = np.array([])
    times = np.array([])
    relVel = np.array([])
    ...
    # find file's time
#    diskname = filename[:-26] + "disk001"
#    IPname = filename[:-11] + "IPDISK" + filenames[-1][-5:-2]
#    Becoords, NScoords, partcoords, partvels, rho, u, num_parts, time, umass, udist, utime = readFILES(filename, diskname, IPname) 
#
#    fileTime = time * utime / (24*3600)
    
    os.system("ls /data/rob1g10/SPH/DATA/DISKS/{}/processedDISKs/QGDISK* > lsDISKSIZE.txt".format(baseName))
    ...
    DISKend, STEPend = calcQDISKsForOrbit(orbit, baseName, DISKstart, DISKsteps)
    
    for i in range(DISKstart, DISKend+5):
        if(i == DISKend):
            end = STEPend
        else:
            end = DISKsteps
        for j in range(1, end):
            filenames = np.append(filenames, '/data/rob1g10/SPH/DATA/DISKS/{}/processedDISKs/QGDISK{:03}{:02}'.format(baseName, i, j))
            ...
            Acoords = readQADISK('/data/rob1g10/SPH/DATA/DISKS/{}/processedDISKs/QADISK{:03}{:02}'.format(baseName, i, j))
    ...
    print len(times), len(relVel)
    periastron = times[np.argmin(NSx)]
#    sortOrder = np.argsort(  ((periastron - times) % orbit ) / orbit )
    plt.plot( ( ( (periastron - times)) / orbit ) , 
               Aparts, 
               color=colour)
    return ( (periastron - times) % orbit ) / orbit#fileTime
...
def readQDISK(filename):
    ...
    Bevels = np.array([xvels[0], yvels[0], zvels[0]])
    NSvels = np.array([xvels[1], yvels[1], zvels[1]])
#    print "Be coords:", Becoords
#    partcoords -= Becoords
#    Becoords -= Becoords
#    NScoords -= Becoords
    return Becoords, NScoords, partcoords[2:], partvels[:], rho[2:], u[2:], num_parts-2
...
def readQADISK(filename):
    ...
    for line in f:
        line = line.strip()
        columns = line.split()
        Acoords[count] = [float(columns[0]), float(columns[1]), float(columns[2])]
        count += 1
...
#filename = '/data/rob1g10/SPH/DATA/DISKS/BH28d_0.2e/processedDISKs/QGDISK43112'
#filename='/data/rob1g10/SPH/DATA/DISKS/M1.4_28d_0.2e/processedDISKs/QGDISK30902'
""" Read File """
#diskname = filename[:-26] + "disk001"
#IPname = filename[:-11] + "IPDISK" + filename[-5:-2]
...
#Becoords, NScoords, partcoords, partvels, rho, u, num_parts, time, umass, udist, utime = readFILES(filename, diskname, IPname)
#Acoords = readQADISK('/data/rob1g10/SPH/DATA/DISKS/M1.4_28d_0.2e/processedDISKs/QADISK30902')
#plt.scatter(NScoords[0], NScoords[1], color='b')
#plt.scatter(Becoords[0], Becoords[1], color='g')
#plt.scatter(Acoords[0], Acoords[1], color='r')
#BeA1 = 464802
#BeA2 = 464969
#NSA1 = 420
#NSA2 = 420
...
def calcQDISKsForOrbit(orbitalPeriod, baseName, QDISKstart, QDISKstepsMAX):
    ...
    for i in range(QDISKstart, QDISKstart+50):
...
#            print filename
            if(np.abs(times[0]-times[-1]) % orbitalPeriod < tol and np.abs(times[0]-times[-1]) / orbitalPeriod >= 5):
                end = [i, j]
                numOrbits = int(np.abs(times[0]-times[-1]) / orbitalPeriod)
                return end#, np.mean(maxRhos), np.mean(nparts)

#print calcQDISKsForOrbit(20, 'M1.4_20d_0.0e', 362, 5)
#print calcQDISKsForOrbit(17.4, 'M6_17.4d_0.0e', 125, 5)

#DISKstart = np.array([362, 373, 370, 255, 125, 285, 418, 320,
#                      300, 318, 290, 239, 350, 128, 290, 360,
#                      351, 168, 139, 123, 119, 104, 163, 141])
#DISKend = np.array([369, 125, 320,
#                    300, 350, 360,
#                    354, 62, 142])
#STEPstart = np.array([ 5, 13, 14,
#                      19, 17, 16,
#                       3, 44, 34])
#STEPend = np.array([ 2, 45, 47,
#                    52, 50, 52,
#                     2, 26, 17])
#DISKsteps = np.array([ 5, 53, 41, 42, 53, 40, 49, 49, 
#                      67, 58, 53, 58, 58, 56, 51, 60,
#                      11, 19, 25, 37, 47, 48, 44, 49])
...
baseName = np.array(['M1.4_20d_0.0e', 'M3_19d_0.0e',   'M4_18.4d_0.0e', 'M5_17.9d_0.0e', 'M6_17.4d_0.0e', 'M7_17d_0.0e',   'M8_16.6d_0.0e', 'M10_15.8d_0.0e', 
                     'M1.4_28d_0.2e', 'M3_26.5d_0.2e', 'M4_25.7d_0.2e', 'M5_25d_0.2e',   'M6_24.3d_0.2e', 'M7_23.7d_0.2e', 'M8_23.1d_0.2e', 'M10_22.1d_0.2e',
                     'M1.4_43d_0.4e', 'M3_40.8d_0.4e', 'M4_39.6d_0.4e', 'M5_38.5d_0.4e', 'M6_37.5d_0.4e', 'M7_36.5d_0.4e', 'M8_35.6d_0.4e', 'M10_34.1d_0.4e'])
#discSize = np.array([])
#discMax = np.array([])
#discMin = np.array([])
#rho0Ave = np.array([])
#rho0Max = np.array([])
#rho0Min = np.array([])
#for k in range(1,len(baseName)):
#    filenames = np.array([])
#    rho0s = np.array([])
#
#    os.system("ls /data/rob1g10/SPH/DATA/DISKS/{}/processedDISKs/QGDISK* > lsDISKSIZE.txt".format(baseName[k]))
#
#    f = open("lsDISKSIZE.txt", 'r')
#    lines = f.readlines()
#	
...
#    f.close()    
#    
#    DISKend, STEPend = calcQDISKsForOrbit(orbit[k], baseName[k], DISKstart, DISKsteps)
#    
##    if(k>0):
##        for num in range(9):
...
#    method = 'cutoff'
#    discSize = np.append(discSize, np.mean(findDiskSize(filenames, method)))
#    discMax = np.append(discMax, np.max(findDiskSize(filenames, method)))
#    discMin = np.append(discMin, np.min(findDiskSize(filenames, method)))
#    
#    rho0Ave = np.append(rho0Ave, np.mean(rho0s))
#    rho0Max = np.append(rho0Max, np.max(rho0s))
#    rho0Min = np.append(rho0Min, np.min(rho0s))

#plt.scatter(Mx[16:24]/2e30, discSize[16:24], marker='^', color='g', label="0.4e")
#plt.errorbar(Mx[16:24]/2e30, discSize[16:24], color='g', yerr=[discMin[16:24], discMax[16:24]])
#
#plt.scatter(Mx[8:16]/2e30, discSize[8:16], marker='x', color='r', label="0.2e")
#plt.errorbar(Mx[8:16]/2e30, discSize[8:16], color='r', yerr=[discMin[8:16], discMax[8:16]])
#
#plt.scatter(Mx[0:8]/2e30, discSize[0:8], marker='o', color='b', label="0.0e")
#plt.errorbar(Mx[0:8]/2e30, discSize[0:8], color='b', yerr=[discMin[0:8], discMax[0:8]])

#plt.scatter(Mx[16:24]/2e30, rho0Ave[16:24], marker='^', color='g', label="0.4e")
#plt.errorbar(Mx[16:24]/2e30, rho0Ave[16:24], color='g', yerr=[rho0Min[16:24], rho0Max[16:24]])
#
#plt.scatter(Mx[8:16]/2e30, rho0Ave[8:16], marker='x', color='r', label="0.2e")
#plt.errorbar(Mx[8:16]/2e30, rho0Ave[8:16], color='r', yerr=[rho0Min[8:16], rho0Max[8:16]])
#
#plt.scatter(Mx[0:8]/2e30, rho0Ave[0:8], marker='o', color='b', label="0.0e")
#plt.errorbar(Mx[0:8]/2e30, rho0Ave[0:8], color='b', yerr=[rho0Min[0:8], rho0Max[0:8]])

#plt.legend(loc='best')
#plt.ylim(0, 8e-11)
#plt.xlabel('Compact Object Mass')
##plt.ylabel('rho0 in gcc')
#plt.ylabel('Size of disc in stellar radii')




#M1.4_20d_0.0e 9.34496185141
#M4_18.4d_0.0e 6.79111103628
#M6_17.4d_0.0e 6.43417567368
#M8_16.6d_0.0e 6.79254362791
#M10_15.8d_0.0e 6.0038530793
#M1.4_28d_0.2e 9.87757604912
#M4_25.7d_0.2e 7.41369100572
#M6_24.3d_0.2e 6.48824271039
#M8_23.1d_0.2e 6.6081560996
#M10_22.1d_0.2e 6.05448715052
#M1.4_43d_0.4e 12.9974279454
#M4_39.6d_0.4e 9.34128125537
#M6_37.5d_0.4e 6.54423984746
#M8_35.6d_0.4e 7.57369259273
#M10_34.1d_0.4e 6.31058968542
