...
#        plotAccFOLD /= numOrbits
        ...
        conversion = (1e-15 * umass / (24*3600)) * (3e8**2 / 2)
...
#        plt.plot(plotPhase - int(np.min(plotPhase)) - 1, plotAcc, color=colour[BN], label='M={}, e={}'.format(Mx[BN], baseNames[BN][-4:-1]))
        plt.plot(accPhase-int(np.max(accPhase))+0.14, accArray, color=colour[BN], label='M={}, e={}'.format(Mx[BN], baseNames[BN][-4:-1]))
...
#        plt.scatter(range(len(binaryPhase)), binaryPhase, color=colour[BN], label='{}'.format(Mx[BN]))
        ...
        Lmax = np.append(Lmax, np.max(accArray))
        Lave = np.append(Lave, np.mean(accArray))
        LaveSQRT = np.append(LaveSQRT, np.mean(np.sqrt(accArray)))
...
#                         '/data/rob1g10/SPH/DATA/DISKS/M1.4_43d_0.4e/processedDISKs/QGDISK49103',
...
#                         '/data/rob1g10/SPH/DATA/DISKS/M6_68.8d_0.6e/processedDISKs/QGDISK24510',
#                         
...
#Lmax, Lave, LaveSQRT, aveRhos, minRhos, maxRhos, aveMinU, minV, discSize, minDiscSize, maxDiscSize = 
plotCOaccretion(baseNames, Mx, orbit, colour = ['b', 'g', 'r', 'y', 'black', 'brown', 'turquoise', 'pink'])
...
#plt.errorbar(Mx[sec5:], np.log10(Lave[sec5:]*conversion), 
#             yerr= np.log10(Lave[sec5:]*conversion) - np.log10(LaveSQRT[sec5:]*conversion),
#             marker='<', ms=10, color='purple', label="e = 0.6")
...
#             marker='s', ms=10, color='black', label="e = 0.4")
...
#             marker='^', ms=10, color='g', label="e = 0.2")
##plt.scatter(Mx[sec2:sec3], Lave[sec2:sec3], marker='x', color='r', label="e = 0.1")
#plt.errorbar(Mx[:sec2], np.log10(Lave[:sec2]*conversion), 
#             yerr= np.log10(Lave[:sec2]*conversion) - np.log10(LaveSQRT[:sec2]*conversion),  
#             marker='o', ms=10, color='b', label="e = 0.0")
##plt.ylabel(r"Average number of particles captured around an orbit")
...
##plt.ylim(0, 12 * (1e-15 * umass / (24*3600)) * (3e8**2 / 2))

#plt.errorbar(Mx[sec5:], np.log10(Lmax[sec5:][::-1]*conversion), 
#             yerr= np.log10(Lmax[sec5:]*conversion) - np.log10(np.sqrt(Lmax[sec5:])*conversion),
#             marker='<', ms=10, color='purple', label="e = 0.6")
...
##print p6[0], p6[1]
##plt.scatter(Mx[sec5:], np.log10(aveRhos[sec5:]), marker='<', color='purple', label="e = 0.6")
#plt.errorbar(Mx[sec5:], np.log10(aveRhos[sec5:]), yerr=[np.log10(aveRhos[sec5:])-np.log10(minRhos[sec5:]), np.log10(maxRhos[sec5:])-np.log10(aveRhos[sec5:])], color='purple', label="e = 0.6", marker='<', ms=10)#, ls='None')
#
...
##plt.scatter(Mx[sec4+1:sec5], np.log10(aveRhos[sec4+1:sec5]), marker='s', color='black', label="e = 0.4")
#plt.errorbar(Mx[sec4:sec5], np.log10(aveRhos[sec4:sec5]), yerr=[np.log10(aveRhos[sec4:sec5])-np.log10(minRhos[sec4:sec5]), np.log10(maxRhos[sec4:sec5])-np.log10(aveRhos[sec4:sec5])], color='black', label="e = 0.4", marker='s', ms=10)#, ls='None')
#p3 = np.polyfit(Mx[sec3:sec4], np.log10(aveRhos[sec3:sec4]), 1)
#print p3[0], p3[1]
##plt.plot(Mx[sec3:sec4], Mx[sec3:sec4]*p3[0] + p3[1], color='g')
##plt.scatter(Mx[sec3:sec4], np.log10(aveRhos[sec3:sec4]), marker='^', color='g', label="e = 0.2")
##plt.errorbar(Mx[sec3:sec4], np.log10(aveRhos[sec3:sec4]), yerr=[np.log10(aveRhos[sec3:sec4])-np.log10(minRhos[sec3:sec4]), np.log10(maxRhos[sec3:sec4])-np.log10(aveRhos[sec3:sec4])], color='g', ls='None')
##sec4-=1
#p2 = np.polyfit(Mx[sec2:sec3], np.log10(aveRhos[sec2:sec3]), 1)
#print p2[0], p2[1]
##plt.plot(Mx[sec2:sec3], Mx[sec2:sec3]*p2[0] + p2[1], color='r')
##plt.scatter(Mx[sec2:sec3], np.log10(aveRhos[sec2:sec3]), marker='x', color='r', label="e = 0.1")
##plt.errorbar(Mx[sec2:sec3], np.log10(aveRhos[sec2:sec3]), yerr=[np.log10(aveRhos[sec2:sec3])-np.log10(minRhos[sec2:sec3]), np.log10(maxRhos[sec2:sec3])-np.log10(aveRhos[sec2:sec3])], color='r', ls='None')
#
#p1 = np.polyfit(Mx[:sec2], np.log10(aveRhos[:sec2]), 1)
#print p1[0], p1[1]
##plt.plot(Mx[:sec2], Mx[:sec2]*p1[0] + p1[1], color='b')
##plt.scatter(Mx[:sec2], np.log10(aveRhos[:sec2]), marker='o', color='b', label="e = 0.0")
#plt.errorbar(Mx[:sec2], np.log10(aveRhos[:sec2]), yerr=[np.log10(aveRhos[:sec2])-np.log10(minRhos[:sec2]), np.log10(maxRhos[:sec2])-np.log10(aveRhos[:sec2])], color='b', label="e = 0.0", marker='o', ms=10)#, ls='None')
#
##plt.xlim(1, 11)
...
#p5 = np.polyfit(Mx[sec5:], discSize[sec5:], 1)
#print p5[0], p5[1]
##plt.scatter(Mx[sec5:], discSize[sec5:], marker='<', color='purple', label="e = 0.6")
#plt.errorbar(Mx[sec5:], discSize[sec5:], yerr=[discSize[sec5:]-minDiscSize[sec5:], maxDiscSize[sec5:]-discSize[sec5:]], color='purple', label="e = 0.6")#, ls='None')
#
#p4 = np.polyfit(Mx[sec4:sec5], discSize[sec4:sec5], 1)
#print p4[0], p4[1]
##plt.plot(Mx[sec4:sec5], Mx[sec4:sec5]*p4[0] + p4[1], color='black')
##plt.scatter(Mx[sec4:sec5], discSize[sec4:sec5], marker='s', color='black', label="e = 0.4")
#plt.errorbar(Mx[sec4:sec5], discSize[sec4:sec5], yerr=[discSize[sec4:sec5]-minDiscSize[sec4:sec5], maxDiscSize[sec4:sec5]-discSize[sec4:sec5]], color='black', label="e = 0.4")#, ls='None')
#
#p3 = np.polyfit(Mx[sec3:sec4-1], discSize[sec3:sec4-1], 1)
#print p3[0], p3[1]
##plt.plot(Mx[sec3:sec4], Mx[sec3:sec4]*p3[0] + p3[1], color='g')
##plt.scatter(Mx[sec3:sec4], discSize[sec3:sec4], marker='^', color='g', label="e = 0.2")
##plt.errorbar(Mx[sec3:sec4], discSize[sec3:sec4], yerr=[discSize[sec3:sec4]-minDiscSize[sec3:sec4], maxDiscSize[sec3:sec4]-discSize[sec3:sec4]], color='g', ls='None')
#
...
##plt.errorbar(Mx[sec2:sec3], discSize[sec2:sec3], yerr=[discSize[sec2:sec3]-minDiscSize[sec2:sec3], maxDiscSize[sec2:sec3]-discSize[sec2:sec3]], color='r', ls='None')
#
#p1 = np.polyfit(Mx[:sec2], discSize[:sec2], 1)
#print p1[0], p1[1]
##plt.plot(Mx[:sec2], Mx[:sec2]*p1[0] + p1[1], color='b')
##plt.scatter(Mx[:sec2], discSize[:sec2], marker='o', color='b', label="e = 0.0")
#plt.errorbar(Mx[:sec2], discSize[:sec2], yerr=[discSize[:sec2]-minDiscSize[:sec2], maxDiscSize[:sec2]-discSize[:sec2]], color='b', label="e = 0.0")#, ls='None')
#
...
eccSMCTownsend = np.array([0.07, 0.26, 0.41, 0.4])
LmaxSMCTownsend = np.array([2.1e38, 3.3e37, 7.3e37, 3.5e37])

eccGalacticRaguzova = np.array([0.03, 0.03, 0.036, 0.09, 0.12, 0.26, 0.3, 0.33, 0.34, 0.4, 0.41, 0.446, 0.47, 0.537, 0.66, 0.88])
LmaxGalacticRaguzova = np.array([1.1e37, 2.1e37, 1.1e38, 7e36, 1.1e37, 3.9e34, 1.1e38, 5.4e36, 3e37, 3e36, 1e38, 8e36, 2e37, 2e34, 2.9e35, 6e36])
eccLMCRaguzova = np.array([])
LmaxLMCRaguzova = np.array([])
eccSMCRaguzova = np.array([])

eccGalactic = np.append(eccGalacticCoe, eccGalacticRaguzova)
...
#Galactic = plt.errorbar(eccGalacticRaguzova, np.log10(LmaxGalacticRaguzova), label="Galactic", color='g', marker='<', ms=10, linestyle='None')
#MWC656 = plt.errorbar(0.1, np.log10(3.7e31), label="MWC656", color='r', marker='s', ms=10, linestyle='None')
#plt.ylabel(r"log L$_{max}$ (erg s$^{-1}$)")
#plt.xlabel("Eccentricity")
