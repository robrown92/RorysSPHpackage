...
#                axarr[1].plot([kepAngstroms for i in range(20)], np.linspace(0, max(countsPlot), 20), color='black')
                axarr[1].hist(angstroms, bins=BINS)
...
plotFeatureVels("BASIC", ['/data/rob1g10/SPH/DATA/DISKS/20d_0.4e_eqTest/processedDISKs/QGDISK38711'],
                        "20d_0.4e_eqTest", [45, 91, 100], [0, 9, 10], 6, 100, "uniform", "bleh")
...
CcodeNames = ["BASIC"]#, "TESTALL", "BASICNOBE", "TESTRAD", "TESTNEW", "TESTEXPH", "TESTEXP", "TESTBOTH", "TESTION"] 
for k in range(len(CcodeNames)):
    ...
    for theta in range(0, 91, 10):
        for i in range(1):
            for j in range(1, 2):
                ...
                oD = readOptDepthData4py("DATA/optDepthData{}{}_theta{}_phi{}_time{:0.1f}.txt".format(CcodeName, baseName, theta, phi, time * utime / (24 * 3600)))
                print(CcodeName, ':', max(oD), min(oD))                
                T = np.exp(-oD)
                Tsum = 0
                for j in range(len(T)):
                    if(R[j]<10 and j != 11613 and j != 30074):
                        Tsum += T[j]
    ...
    #EWs1 /= 1000
#    print EWs1
    plt.plot(range(0, 91, 10), EWs1[k*10:(k+1)*10], label=CcodeName)#, label="1")
...
#    for i in range(5):
#        for j in range(1, 25):
#            filename = '/data/rob1g10/SPH/DATA/DISKS/20d_0.0e_eqTest/processedDISKs/QGDISK{}{:02}'.format(387+i, j)
#            diskname = filename[:-26] + "disk001"
#            IPname = filename[:-11] + "IPDISK" + filename[-5:-2]
#            Becoords, NScoords, partcoords, partvels, rho, u, num_parts, time, umass, udist, utime = readFILES(filename, diskname, IPname)
#            Becoords *= udist / (7 * 6.96e10)
...
#            partcoords *= udist / (7 * 6.96e10)
#            #R = np.sqrt(partcoords[:,0]**2 + partcoords[:,1]**2)
#            baseName = "20d_0.0e_eqTest"
#            oD = readOptDepthData4py("/data/rob1g10/SPH/DATA/DISKS/20d_0.0e_eqTest/optDepthData{}_theta{}_phi{}_time{:0.1f}.txt".format(baseName, theta, phi, time * utime / (24 * 3600)))
#            T = np.exp(-oD)
